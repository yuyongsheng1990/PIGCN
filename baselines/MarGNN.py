...
class MarGNN(nn.Module):
    ...
    def forward(self, x, adjs, n_ids, device, RL_thresholds):  # entity, userid, word
        ...
        for i in range(self.num_relations):  # i: 0, 1, 2
            if not self.is_shared:
                features.append(self.intra_aggs[i](x[n_ids[i]], adjs[i], device))  # batch node neighbors -> (100, 64)
            else:
